...
# All features
x_train_model2, x_test_model2, y_train_model2, y_test_model2 = train_test_split(x_train, y_train, test_size=0.33,
                                                                                random_state=42)
...
r_scaler = preprocessing.MinMaxScaler()  # Scale the non-normalized df_train to between 0-1
r_scaler.fit(df_train)
modified_data = pd.DataFrame(r_scaler.transform(df_train), columns=df_train.columns)
print(modified_data.head)
selector = SelectKBest(chi2, k=10)
x_train = modified_data.drop(labels="satisfaction",
                             axis=1)  # df with only satisfaction and df with every other variable
...
x_train = df_train_important2.drop(labels="satisfaction",
                                   axis=1)  # df with only satisfaction and df with every other variable
y_train = df_train_important2["satisfaction"]
#%%
x_train_model, x_test_model, y_train_model, y_test_model = train_test_split(x_train, y_train, test_size=0.25,
                                                                            random_state=42)
...
print("test", len(test))
#%%
# Log Regression 10 important features
...
print("test", len(test))
#%%
# Log reg 10 important features minus on-board and leg-room service
logreg = LogisticRegression()
logreg.fit(x_train_model, y_train_model)
#%%
# importance graph
...
print(result.summary())
#%%
# Let's make Dummy variables for the variables and feature select from them
...
#%%
# Chi2, top 10 features
# Let's use chi2 test of importance to find the top 10 features
...
x_train = df_train_important.drop(labels="satisfaction",
                                  axis=1)  # df with only satisfaction and df with every other variable
...
print("test", len(test))
#%%
# Initiate the Logistic Regression Model, importance plot, and print the accuracy
logreg = LogisticRegression()
logreg.fit(x_train_model, y_train_model)
#%%
importance = logreg.coef_[0]
...
x_train = df_train_important.drop(labels=["satisfaction", 'Ease of Online booking_5'],
                                  axis=1)  # df with only satisfaction and df with every other variable
y_train = df_train_important["satisfaction"]
...
print("test", len(test))
#%%
# Initiate the Logistic Regression Model, plot importance and print the accuracy
logreg = LogisticRegression()
logreg.fit(x_train_model, y_train_model)
#%%
importance = logreg.coef_[0]
